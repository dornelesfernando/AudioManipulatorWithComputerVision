    ...
    def find_positions(self, 
        ...
        if self.results and self.results.multi_hand_landmarks:
            for hand_landmarks in self.results.multi_hand_landmarks:
                ...
                all_hands.append(hand_landmark_list)
        return all_hands
...
if __name__ == '__main__':
    ...
    while True:
        ...
        cv2.putText(img, f"Vol {int(volPercentAux)}%", (30, 90), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 1)
        ...
        if hands:
            ...
            if len(hands) == 2:
                h1x1, h1y1 = hands[0][4][1], hands[0][4][2]
                h1x2, h1y2 = hands[0][8][1], hands[0][8][2]
                h1cx, h1cy = (h1x1 + h1x2) // 2, (h1y1 + h1y2) // 2

                h2x1, h2y1 = hands[1][4][1], hands[1][4][2]                
                h2x2, h2y2 = hands[1][8][1], hands[1][8][2]  
                ...
                # print(length)
                vol = np.interp(length, [30, 400], [minVol, maxVol])
                volPercentAux = volPercent = np.interp(length, [30, 400], [0, 100])

                # print(int(length), vol)
                volume.SetMasterVolumeLevel(vol, None)

                if length < 30:
                    cv2.line(img, (h1cx, h1cy), (h2cx, h2cy), (0, 0, 0), 3)
        ...
        if cv2.waitKey(20) & 0xFF == (ord('q') or ord('Q')):
            break 
